...
```{r,eval=FALSE}
install.packages(c('rjags', 'R2jags', 'devtools', 'tidyverse', 'vegan', 'ellipse', 'MCMCglmm'))
...
The steps above do not require any unusual package installations. The main thing to watch for is that you have installed JAGS directly from the sourceforge website, and the R packages rjags and R2jags which are the interfaces between R and JAGS. Installing the R packags **does not** install the actual JAGS software and vice versa.

The other main problem will be out of date Rstudio and R installations.

If you run into any other problems please drop us a line at <a.jackson@tcd.ie> or <KELLYR44@tcd.ie>.
